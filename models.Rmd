...
```
```{r}
coef_abs <- abs(coef(full_model)[-1]) # Remove intercept
coef_importance <- data.frame(Feature = names(coef_abs), Importance = coef_abs,
                              row.names = NULL)
coef_importance <- coef_importance %>% arrange(desc(Importance))
print(coef_importance)
```
```{r}
coef_abs <- abs(coef(forward_model)[-1]) # Remove intercept
coef_importance <- data.frame(Feature = names(coef_abs), Importance = coef_abs,
                              row.names = NULL)
coef_importance <- coef_importance %>% arrange(desc(Importance))
print(coef_importance)
```
```{r}
coef_abs <- abs(coef(backward_model)[-1]) # Remove intercept
coef_importance <- data.frame(Feature = names(coef_abs), Importance = coef_abs,
                              row.names = NULL)
coef_importance <- coef_importance %>% arrange(desc(Importance))
print(coef_importance)
